function managerTemplate(manager) {
    ...
    <div class="card">
        <div class="card-header" style="background-color: blue; color:aqua;">
            <h2>
            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-cup-hot-fill" viewBox="0 0 16 16">
                              ...
                              </svg>  Manager <br>${manager[0].getName()}</br></h2>
        </div>
...
function engineerTemplate(engineer) {
    const engineerArr = engineer.map(engineerObject => {
        return ` <article class=" col-md-4 pb-3">
        <div class="card">
            <div class="card-header" style="background-color: blue; color:aqua;">
                <h2><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-emoji-sunglasses" viewBox="0 0 16 16">
              ...
              </svg>  Engineer <br>${engineerObject.getName()}</br></h2>
            </div>
    
            <ul class="list-group list-group-flush">
                <li class="list-group-item">ID: ${engineerObject.getId()}</li>
                <li class="list-group-item">Email: ${engineerObject.getEmail()}</li>
                <li class="list-group-item">GitHub: ${engineerObject.getGitHub()}</li>
            </ul>
        </div>
    </article>`
    })
    
    console.log(engineerArr.join(''));

    return engineerArr.join('');
};

function internTemplate(intern) {
    return intern.map(internObject => {
            return ` <article class=" col-md-4 pb-3">
            <div class="card">
                <div class="card-header" style="background-color: blue; color:aqua;">
                    <h2><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-mortarboard-fill" viewBox="0 0 16 16">
                  ...
                  </svg>  Intern <br>${internObject.getName()}</br></h2>
                </div>

                <ul class="list-group list-group-flush">
                    <li class="list-group-item">ID: ${internObject.getId()}</li>
                    <li class="list-group-item">Email: ${internObject.getEmail()}</li>
                    <li class="list-group-item">School: ${internObject.getSchool()}</li>
                </ul>
            </div>
        </article>`
    }).join('')
...
function template(manager, engineer, intern) {
